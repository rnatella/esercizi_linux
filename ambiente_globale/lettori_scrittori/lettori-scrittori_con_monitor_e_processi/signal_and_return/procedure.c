...
void FineLettura(Monitor* m, Buffer * buf) {
		...
		if (buf->numlettori==0) {// se sono finiti i lettori lo segnala agli scrittori
			...
			/* NOTA: nella semantica signal-and-return, l'operazione di signal_condition()
			 *       effettua *anche* l'uscita dal monitor (leave_monitor). Quindi, la
			 *       leave_monitor() va chiamata *solo se* non è stata già chiamata signal_condition().
			 *       Per questo motivo, leave_monitor() è eseguita solo nel ramo "else" */
...
void FineScrittura(Monitor* m, Buffer*buf){
	...
	{
		signal_condition(m,SYNCHS); // sblocco di eventuali altri scrittori

    } else if (queue_condition(m,SYNCHL)>0) 
	{
     	signal_condition(m,SYNCHL); // sblocco di eventuali altri lettori
	}
    else {

		/* NOTA: come in FileLettura(), la leave_monitor() va chiamata *solo se*
		 *       non è stata già chiamata la signal_condition() */

		leave_monitor(m);	// lascia il monitor
	}
